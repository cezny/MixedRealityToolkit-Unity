...
namespace Microsoft.MixedReality.Toolkit.Input
{
    ...
    public class FocusProvider : BaseDataProvider, IMixedRealityFocusProvider
    {
            ...
            for (int i = 0; i < pointerRays.Length; i++)
            {
                switch (pointer.SceneQueryType)
                {
                    case SceneQueryType.SimpleRaycast:
                        ...
                        {
                            UpdatePointerRayOnHit(pointerRays, in physicsHit, i, rayStartDistance, hit);
                            return;
                        }
                        break;
        ...
        private static void UpdatePointerRayOnHit(RayStep[] raySteps, in RaycastHit physicsHit, int hitRayIndex, float rayStartDistance, PointerHitResult hit)
        {
